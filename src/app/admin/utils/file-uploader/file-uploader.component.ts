...
export class FileUploaderComponent implements OnInit {
  ...
  uploadFiles() {
    ...
    let formData = new FormData();
     ...
     formData.append('parent', this.config["parent"] || null)

     formData.append('assets', this.config["assets"] || 'uses')
    ...
    xhr.onreadystatechange = evnt => {
      ...
      if (xhr.readyState === 4) {
        if (xhr.status !== 200) {
          ...
          this.uploadMsgText = "Upload falhou!";
    ...
    xhr.onload = evnt => {
      ...
      this.afterUpload = true;
      if (!isError) {
        this.uploadMsgText = "Upload Realizado Com Sucesso!";
    ...
    xhr.setRequestHeader('Company-key', APP_KEY_CONPANY);

    xhr.setRequestHeader('Authorization', `Bearer ${this.jwtToken.token}`);
